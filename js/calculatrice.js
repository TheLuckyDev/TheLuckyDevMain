(function(window) {
	...
	var calcSS3 = document.querySelector('.calc-main'),
		...
		historyElement.innerHTML = "<div class='calcul' style='text-align=top'><div></div></div>";
	...
	function saveToHistory(key){
		if(key != '='){
			historyText+=key;
			var t=historyText.replace(/m\+/g,"");
			t=historyText.replace(/AC/g,"");
			t=historyText.replace(/A/g,"");
			t=historyText.replace(/m-/g,"");
			t=historyText.replace(/C/g,"");
			historyElement.innerHTML = "<div class='calcul' style='text-align=top;height:auto'><div style='line-height:35px'>"+t+"</div></div>";
			//console.log(t);
		}
		if(key ==='='){
      		var text = display.innerHTML;
      		var calculs = document.getElementsByClassName("calcul");
      		let div = document.createElement("DIV","resultat");
      		calculs[0].appendChild(div);
      		let t = document.createTextNode(text);
            div.style.lineHeight="16px";
      		div.style.fontSize='15px';
    		div.style.width = '100%';
            div.style.maxHeight='17px';
    		div.style.color='red';
      		div.appendChild(t);
      		//calculs.appendChild(div);
      		//historyText = '';
     	}

	}
    function resultToHistory(){
      	var text = display.innerHTML;
      	var calculs = document.getElementsByClassName("calcul");
      	let div = document.createElement("DIV","resultat");
      	calculs[0].appendChild(div);
    ...
    function resetHistory(){
	   historyElement.innerHTML = "<div class='calcul' style='text-align=top;height:auto'><div style='line-height:35px'></div></div>";
        //calculs.appendChild(div);
        historyText = '';
	}
    
    function firstCharacterHistory(operator){
        console.log(historyText);
        historyElement.innerHTML = "<div class='calcul' style='text-align=top;height:auto'><div style='line-height:35px'>"+operator+"("+historyText+")</div></div>";
        resultToHistory();
	}
...
console.log("NOTICE : \n\
            * Bug lors de l'utilisation du bouton opposé (+/-), le caractère s'affiche alors qu'l ne devrait pas\n\
            * Bug si l'on a pas valid le résultat avant d'utiliser une méthode scientifique (sqrt ou log ect...)");